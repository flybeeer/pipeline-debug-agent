...
## วิธีป้อนให้ agent — auto-load แล้ว ✅

`new_state()` จะ **auto-load** `runbooks/<issue_id>.md` เข้า `state.runbook` ให้เอง
(ผ่าน `integrations/runbook.py:load_runbook`) — ไม่ต้องอ่านไฟล์เองอีก:

```python
state = new_state(issue_id="null-userid-042", error_log=..., ...)
# state["runbook"] = เนื้อหา runbooks/null-userid-042.md (ถ้ามี)
# state["has_runbook"] = True → ดู nodes/tiering.py
```

- ไม่มีไฟล์ → `runbook=""`, `has_runbook=False` (ปัญหาใหม่ → ปลอดภัยไว้ก่อน)
- ส่ง `runbook=""` มาตรงๆ = บังคับปิด auto-load (override)
- เปลี่ยนที่เก็บได้ด้วย env `RUNBOOKS_DIR` (default `runbooks`)
