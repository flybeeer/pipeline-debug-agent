...
> 📒 คู่มือแก้ปัญหาต่อ issue (T2/T3) → [`runbooks/`](./runbooks/) (เริ่มจาก `TEMPLATE.md`)
...
เห็น loop เต็มวง `Action→Check→Repeat→Review` โดยใช้ fake LLM:
runner รัน fix SQL ลง DuckDB (engine harness) → validation อ่าน output ผ่าน Iceberg adapter
(catalog fake อ่าน duckdb → Arrow) ตรวจ semantic → tiering ได้ T1 → เปิด PR (stub) auto-merge
...
4. ✅ `integrations/validation.py` + `integrations/warehouse.py` — semantic check
   อ่าน output จาก **Apache Iceberg** (PyIceberg + PyArrow, read-only), spec ต่อ issue
   ที่ `config/checks/<issue_id>.yaml` (ดู `config/checks/example.yaml`)
   warehouse อื่น: เพิ่ม adapter ใน `_ADAPTERS` ของ `warehouse.py`
